...
def driver(trainfileName,testFileName,n):
    ...
    model = perceptron.Perceptron(None, n+1)
...
def withoutMargin(d1, l1, d2, l2, model):
    callPerceptron(d1, l1, d2, l2, 1, 0, model)

# function to identify the best learning rate for the given data
def withMargin(d1, l1, d2, l2, model):
    gamma = [1]
    eta = [1.5, .25, .03, .005, .001]
    accuracy = 0.0
    for i in gamma:
        for j in eta:
            tempAccuracy = callPerceptron(d1, l1, d2, l2, j, i, model)
            print tempAccuracy
            print i, " ", j
...
def callPerceptron(d1, l1, d2, l2, eta, gamma, model):
